...
# Only prepares the cases without submitting the jobs.
dry_run = True

runDir = "calcs"
grid_dir = "./grids_refresco"
nCoresPerNode = 20
maxNodesPerJob = 5
...
# Read the grid statistics and filenames.
grids = pandas.read_csv(os.path.join(grid_dir, "gridStats.csv"))

# Define case permutations. 
caseDefinitions = {
    #"LDCF": {
    #    "BCs": {
    #        "x_neg": {"bcType": "BCWall"},
...
# Define additional variables to modify in the controls file for each case.
additionalControls = {
    "equations/equation/EQMomentum/relaxationProcedure/IMPLICIT_EXPLICIT/imp_relax_min": \
        [str(v) for v in [0.05, 0.25, 0.50, 0.92]],
    "equations/equation/EQMomentum/convectiveFluxDiscretisation": \
        [
            "<FOU_CDS_BLEND><blendingFactor>0.0</blendingFactor></FOU_CDS_BLEND>",
            "<FOU_CDS_BLEND><blendingFactor>0.97</blendingFactor></FOU_CDS_BLEND>",
            "<TVD_SCHEME><schemeName>LIMITED_QUICK</schemeName></TVD_SCHEME>",
            "<TVD_SCHEME><schemeName>HARMONIC</schemeName></TVD_SCHEME>",
        ],
}
 
# Prepare a file for grabbing the slurm outputs.
...
for caseName in caseDefinitions:
    ...
    # Generate all permutations of additionalControls
    addControlKeys = list(additionalControls.keys())
    addControlValues = list(itertools.product(*additionalControls.values()))
    ...
    for iGrid in grids.index:
        ...
        # TODO temp
        if (grids.loc[iGrid, "nCells"] > 20e3):# or ("3D" in grids.loc[iGrid, "grid"]):
        #if ("3D" in grids.loc[iGrid, "grid"]):
            continue
        # TODO temp

        # See if this grid belongs with this case.
        if "gridNameRegex" in caseDefinition:
            if len(re.findall(caseDefinition["gridNameRegex"], grids.loc[iGrid, "grid"])) == 0:
                continue

        # Loop over all control permutations
        for iControlsPermutation, controlCombination in enumerate(addControlValues):

            currentCase = os.path.join(runDir, "case_{:s}_grid_{:d}_controlsPerm_{:d}".format(caseName, iGrid, iControlsPermutation))
            print("Preparing:", currentCase, grids.loc[iGrid, "grid"], " permutation:", iControlsPermutation)
            ...
            try:
                ...
                # Apply the current permutation of additional controls
                for key, value in zip(addControlKeys, controlCombination):
                    element = root.find(key)
                    if element is None:
                        # Create the parent element if it doesn't exist
                        parent = root.find("/".join(key.split("/")[:-1]))
                        newElement = ElementTree.Element(key.split("/")[-1])
                        parent.append(newElement)
                        element = newElement

                    # Clear existing content and replace with new content
                    element.clear()  # Remove all child elements and text
                    if value.startswith("<") and value.endswith(">"):
                        # Parse the string as XML if it contains complex tags
                        parsedContent = ElementTree.fromstring(value)
                        element.append(parsedContent)
                    else:
                        # Set simple text content
                        element.text = value
            
                # Write the controls file.
